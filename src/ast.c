...
{
  if (!left->type || !right->type || !right || !left)
    return 1;
  ...
  if (strcmp(left->type, "IDENTIFIER") == 0)
  {
    ...
    if (symbolID == -1)
      return 1; /* symbol is undeclared (error) */
    symbolkey = &symbolID;
  ...
  if (strcmp(left->astNodeClass, "TERM") == 0)
  {
    if (strcmp(left->type, "INT") == 0)
      ...
      strcpy(leftType, "float");
    else if (strcmp(left->type, "ELEM") == 0)
      strcpy(leftType, "elem");
    else if (strcmp(left->type, "SET") == 0)
      strcpy(leftType, "set");
  }
  if (strcmp(right->astNodeClass, "TERM") == 0)
  {
    if (strcmp(right->type, "INT") == 0)
      strcpy(rightType, "int");
    else if (strcmp(right->type, "FLOAT") == 0)
      strcpy(rightType, "float");
    else if (strcmp(right->type, "ELEM") == 0)
      strcpy(rightType, "elem");
    else if (strcmp(right->type, "SET") == 0)
      strcpy(rightType, "set");
  }
...
void print_parser_ast(parserNode *node, int level)
{
  if (node)
  {
    ...
    print_parser_ast(node->middleBranch, level + 1);
...
void free_parser_ast(parserNode *node)
{
  ...
  if (node->middleBranch)
  {
    free_parser_ast(node->middleBranch);
  }
  if (node->rightBranch)
  {
